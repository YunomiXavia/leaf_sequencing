...
1. \(\bigl|\;y_{i+1} - y_i\bigl|\;\le\;D_{\text{max}}\) với mọi \(i = 1, 2, \ldots, n-1\).
...
Vì bài toán này có thể **tồn tại nhiều nghiệm tối ưu** (cùng giá trị chi phí), **lời giải** thu được có thể **không duy nhất**.
...
Vì các \(y_i\) có thể nhận giá trị **thực** bất kỳ, để triển khai Quy hoạch động (DP), ta thường:

1. **Giới hạn** miền tìm kiếm vào một khoảng \([\min x - \delta,\; \max x + \delta]\).
2. **Chia** khoảng này thành nhiều điểm rời rạc với một bước nhảy (step).
...
- Sau khi tính \(\text{dp}[i][j]\), ta chọn giá trị nhỏ nhất trong số các \(k\) thỏa mãn.
...
Trong quá trình tìm \(\min\), có thể xảy ra tình huống **nhiều** chỉ số \(k\) cùng cho một giá trị \(\text{dp}[i-1][k]\) (chi phí) **bằng nhau**. Lúc này, ta cần quy ước chọn \(k\) như thế nào.
...
- Ở đây, nếu `dp_prev[k] == best_prev` thì ta **giữ nguyên** `best_k`.
- Kết quả: **k đầu tiên** (trong khoảng `[k_left, k_right]`) mà ta gặp có chi phí `dp_prev[k] = best_prev` sẽ được chọn.
...
for (int k = k_left; k <= k_right; k++) {
    if (dp_prev[k] < best_prev) {
        ...
        // Thay vì giữ nguyên, ta đưa ra tiêu chí phụ:
        // 1) Chọn k lớn hơn
        if (k > best_k) {
            best_k = k;
        }
...
> - **Tie-break “chọn k đầu tiên”**: Kết quả có xu hướng “ổn định”.
...
Bạn cũng có thể **đổi chiều duyệt** (từ `M-1` đến `0` thay vì `0` đến `M-1`) để đạt hiệu ứng tương tự.

### Tại sao cần quan tâm “tie-break”?

Vì bài toán **có thể có nhiều** nghiệm tối ưu khác nhau (do các lá có thể dịch chuyển trong giới hạn \(D_{\text{max}}\)), đôi khi bạn muốn “ưu tiên” dãy \(\{y_i\}\) theo một **tiêu chí phụ** (chẳng hạn, chọn \(y_1\) càng gần \(x_1\) càng tốt, hoặc chọn \(\{y_i\}\) càng lớn càng tốt...). Khi đó, cơ chế tie-break sẽ giúp bạn **định hình** nghiệm cuối cùng.

---

## 4. Code C++ chi tiết (có lưu vết & tie-break minh họa)

Dưới đây là ví dụ code C++ hoàn chỉnh. Trong đó, phần **tie-break** được chú thích rõ ràng, bạn có thể **điều chỉnh** để thay đổi kết quả lời giải.
        ...
        for (int j = 0; j < M; j++) {
            ...
            for (int k = k_left; k <= k_right; k++) {
                if (dp_prev[k] < best_prev) {
                    ...
                    // ======== TIE-BREAK VÍ DỤ ========
                    // Nếu cùng chi phí, chọn k lớn hơn (hoặc tùy theo ý)
                    if (k > best_k) {
                        best_k = k;
                    }
                }
            ...
            // Ghi dp_curr[j] và parent
            dp_curr[j] = best_prev + w[i] * fabs(x[i] - yj);
            parent[i][j] = best_k;
        }
        // Chuyển dp_curr -> dp_prev
        dp_prev = dp_curr;
        // Reset dp_curr
...
Trong đoạn `// TIE-BREAK VÍ DỤ`, ta có thể:
...
Việc thay đổi này **không** làm tăng chi phí tối ưu, mà chỉ thay đổi **phân bố \(\{y_i\}\)**. Qua đó, bạn có thể điều khiển “nghiệm” cuối cùng theo nhu cầu riêng.
